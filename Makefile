...
test-integration:
	@CARGO_TARGET_DIR=${CARGO_TARGET_DIR} cargo build ${CARGO_FLAGS}
	@R2_ASM_PLUGIN=${BUILD_DIR}/libc166_asm$(SHARED_EXT) R2_ANALYSIS_PLUGIN=${BUILD_DIR}/libc166_analysis$(SHARED_EXT) CARGO_TARGET_DIR=${CARGO_TARGET_DIR} cargo test --no-fail-fast -Z package-features -p c166-analysis --features integration-tests esil::
